...
let interpolationMode = 'PHONG';  //  'PHONG' or 'GOURAUD'
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            cone.updateNormals();
            shadingMode = 'SMOOTH';
            updateText(textOverlay3, "shading mode: " + shadingMode + " (" + interpolationMode + ")");
            render();
        }
        else if (event.key == 'f') {
...
async function initShader() {
    const vertexShaderSource0 = await readShaderFile('phongShVert.glsl');
    const fragmentShaderSource0 = await readShaderFile('phongShFrag.glsl');
    phongShader = new Shader(gl, vertexShaderSource0, fragmentShaderSource0);

    const vertexShaderSource1 = await readShaderFile('gouraudShVert.glsl');
    const fragmentShaderSource1 = await readShaderFile('gouraudShFrag.glsl');
    gouraudShader = new Shader(gl, vertexShaderSource1, fragmentShaderSource1);
}
...
function render() {
    ...
    // Choose the shader based on the shading mode
    if (interpolationMode == 'GOURAUD') {
        curShader = gouraudShader;
    } else if (interpolationMode == 'PHONG') {
        curShader = phongShader;
    }

    // drawing the cone
    curShader.use();  // using the cylinder's shader
    curShader.setMat4('u_model', modelMatrix);
    curShader.setMat4('u_view', viewMatrix);
    curShader.setVec3('u_viewPos', cameraPos);
    cone.draw(curShader);
...
async function main() {
    try {
        ...
        // set variables in gouraud shader
        gouraudShader.use();
        ...
        // set up text and keyboard events
        setupText(canvas, "Cone with Lighting", 1);
        textOverlay2 = setupText(canvas, "arcball mode: " + arcBallMode, 2);
        textOverlay3 = setupText(canvas, "shading mode: " + shadingMode + " (" + interpolationMode + ")", 3);
